...
class BudgetGraph:
    ...
    def graph_pie(self):
        '''
        Creates a pie chart of the highest prices parts from each catagory
        :returns: A dictionary with the html and js needed to display the chart in a template
        '''
        data = {
    ...
    def create_price_charts(self):
        ...
        def update_charts(cata):
            '''
            Gets the prices for a catagory, creates a line chart for it and
            updates the data_dict.
            :param cata: The name of a catagory to create a line chart for
            :param parameter_2: this is what the second paramete represents
            '''
    ...
    def make_line_chart(self, data):
        '''
        Creates the line chart from the data supplied. 
        :param data: This is a pandas dataframe with all pricing data for a catagory
        :returns: A string with the html and js needed to display the line chart in a template
        '''
        data['float_price'] = (data['price'] * 0.01).round(2)
        data['name'] = [" ".join(x.split()[:5])
                        for x in data['part__long_name']]
        data_list = []
        dates_prices = data.groupby('part_id').agg(
            {'date': list, 'float_price': list, 'name': list})
